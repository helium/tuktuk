...
export async function getCronJobForName(
  program: Program<Cron>,
  name: string,
): Promise<PublicKey | null> {
  const nameMapping = cronJobNameMappingKey(
    program.provider.wallet!.publicKey,
    name,
  )[0];
  const cronJobNameMapping =
    await program.account.cronJobNameMappingV0.fetchNullable(nameMapping);
...
export async function createCronJob(
  ...
  const userCronJobs =
    await program.account.userCronJobsV0.fetchNullable(userCronJobsK);
