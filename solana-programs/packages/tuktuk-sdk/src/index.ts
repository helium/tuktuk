...
export type InitializeTaskQueueArgsV0 =
  IdlTypes<Tuktuk>["initializeTaskQueueArgsV0"];
export type QueueTaskArgsV0 = IdlTypes<Tuktuk>["queueTaskArgsV0"];

export async function getTaskQueueForName(
  program: Program<Tuktuk>,
  name: string,
): Promise<PublicKey | null> {
  const nameMapping = taskQueueNameMappingKey(TUKTUK_CONFIG, name)[0];
  const taskQueueNameMapping =
    await program.account.taskQueueNameMappingV0.fetchNullable(nameMapping);
...
): Promise<MethodsBuilder<Tuktuk, Tuktuk["instructions"][4]>> {
  const tuktukConfig =
    await program.account.tuktukConfigV0.fetch(TUKTUK_CONFIG);
  const nextTaskQueueId = tuktukConfig.nextTaskQueueId;
  return program.methods.initializeTaskQueueV0(args).accounts({
...
export const customSignerSeedsWithBumps = (
  signerSeeds: Buffer[][],
  taskQueue: PublicKey,
) => {
